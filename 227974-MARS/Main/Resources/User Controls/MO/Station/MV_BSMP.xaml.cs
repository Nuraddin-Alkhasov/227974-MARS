...
namespace HMI.Resources.UserControls.MO
{
    public partial class MV_BSMP : UserControl
    {
        ...
        {
            InitializeComponent();
            ManipulatorPosition = "CPU1.PLC.Blocks.02 Basket handling.00 Main.DB BH HMI.Actual.State Picture view Manipulator";
        }
        ...
        private MVBasket GetBasket(string Basket, Thickness margin, int ManiPos) 
        {
            ...
            {
                case "L":
                    temp.IsBasket = "CPU1.PLC.Blocks.02 Basket handling.01 MP.00 Main.DB MP PD.Baskets.isBaskets";
                    temp.IsMaterial = "CPU1.PLC.Blocks.02 Basket handling.01 MP.00 Main.DB MP PD.Charge.isMaterial";
                    temp.SetLayer = "CPU1.PLC.Blocks.02 Basket handling.01 MP.00 Main.DB MP PD.Charge.Layers.Set";
                   
                    temp.ActualLayer = "CPU1.PLC.Blocks.02 Basket handling.01 MP.00 Main.DB MP PD.Charge.Layers.Actual";
                    temp.AS = "Set";
                    if (ManiPos >= 0 && ManiPos <= 2 || ManiPos == 4)
                    {
                        temp.IsDischarge = "CPU1.PLC.Blocks.02 Basket handling.01 MP.00 Main.DB MP PD.Baskets.Functions.Discharge";
                    }
                    break;
                case "R":
                    temp.IsBasket = "CPU1.PLC.Blocks.02 Basket handling.01 MP.00 Main.DB MP PD.Baskets.isBaskets";
