...
namespace HMI.MainRegion.MO.Views
{
    ...
    public partial class MO_CD
    {
        ...
        public MO_CD()
        {
            ...
            DTPosition = VS.GetVariable("CPU1.PLC.Blocks.03 Basket coating.10 DT.DB DT HMI.Actual.Lift.Motor.Actual.Position in units");
            DTPosition.Change += DTPosition_Change;
            IsDT = VS.GetVariable("CPU1.PLC.Blocks.03 Basket coating.10 DT.DB DT HMI.Actual.Dipping Vat.Is Vat");
            IsDT.Change += IsDT_Change;
            VC = VS.GetVariable("CPU1.PLC.Blocks.03 Basket coating.10 DT.DB DT HMI.Actual.Dipping Vat.Viscosity.Check hard required");
            VC.Change += VC_Change;
        ...
        {
            StepType = VS.GetVariable("CPU1.PLC.Blocks.03 Basket coating.00 Main.Stepchains.DB Coating Program control.Set active Step.Data.Type");
        ...
        private void TextVarOut_ValueChanged(object sender, VisiWin.DataAccess.VariableEventArgs e)
        {
            if (e.Quality.Data == DataQuality.Good)
            {
                ...
                {
                    PaintTyp.Value = ApplicationService.GetVariableValue("CPU1.PLC.Blocks.03 Basket coating.10 DT.DB DT Parameter.Name[" + (short)e.Value + "]").ToString();
                }
                else
                {
                    PaintTyp.Value = " - - - ";
