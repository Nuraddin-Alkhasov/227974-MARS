...
namespace HMI.MainRegion.MO
{
    ...
    public partial class MO_PZWZ
    {
        ...
        public MO_PZWZ()
        {
            ...
            Clocked = "CPU2.PLC.Blocks.01 Tray Handling.01 PZ.02 TO.DB TO HMI.PC.Clocked";
        ...
        private void AddObjects_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i <= 10; i++)
            {
                ...
                uint ChargeNr = (uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.01 PZ.02 TO.DB TO PD.Place[" + i + "].Charge.Data.Charge");
                if (ChargeNr == 1)
                {
                    Dispatcher.InvokeAsync(delegate
                    {
                        ...
                        {
                            Data = new Track((uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.01 PZ.02 TO.DB TO PD.Place[" + i + "].Header.BoxId"))
                        };
                        switch (i)
                        {
        ...
        private PZWZTray GetPZWZTray(int i)
        {
            return new PZWZTray()
            {
                IsTray = "CPU2.PLC.Blocks.01 Tray Handling.01 PZ.02 TO.DB TO PD.Place[" + i + "].Tray.isTray", 
                IsMaterial = "CPU2.PLC.Blocks.01 Tray Handling.01 PZ.02 TO.DB TO PD.Place[" + i + "].Charge.isMaterial",
                SetLayer = "CPU2.PLC.Blocks.01 Tray Handling.01 PZ.02 TO.DB TO PD.Place[" + i + "].Charge.Layers.Set",
                ActualLayer = "CPU2.PLC.Blocks.01 Tray Handling.01 PZ.02 TO.DB TO PD.Place[" + i + "].Charge.Layers.Actual",
                
                IsDischarge = "CPU2.PLC.Blocks.01 Tray Handling.01 PZ.02 TO.DB TO PD.Place[" + i + "].Tray.Functions.Discharge",
                IsWatch = "CPU2.PLC.Blocks.01 Tray Handling.01 PZ.02 TO.DB TO PD.Place[" + i + "].Tray.Functions.Watch",
                Station = 12,
                Place = i,
                Type = "Tray",
                CPU = "CPU2",
