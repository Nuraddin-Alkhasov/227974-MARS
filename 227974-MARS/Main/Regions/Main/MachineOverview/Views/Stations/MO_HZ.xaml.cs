...
namespace HMI.MainRegion.MO.Views
{
    ...
    public partial class MO_HZ
    {
        ...
        public MO_HZ()
        {
            ...
            Clocked = "CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ HMI.PC.Clocked";
        ...
        private void AddObjects_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i <= 9; i++)
            {
                ...
                var a = ApplicationService.GetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i + "].Status.Coordinate");
                short Coord = (short)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i + "].Status.Coordinate");
                HZTray T = null;
                Dispatcher.InvokeAsync(delegate
                {
                    T = GetHZTray(i, Coord);
                ...
                uint ChargeNr = (uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i.ToString() + "].PD.Charge.Data.Charge");
                ...
                if (ChargeNr == 1)
                {
                    ...
                    Dispatcher.InvokeAsync(delegate
                    {
                        ...
                        {
                            Data = new Track((uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i.ToString() + "].PD.Header.BoxId"))
                        };
        ...
        private HZTray GetHZTray(int i, int coord)
        {
            return new HZTray()
            {
                IsTray = "CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i + "].PD.Tray.isTray", 
                IsMaterial = "CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i + "].PD.Charge.isMaterial",
                SetLayer = "CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i + "].PD.Charge.Layers.Set",
                ActualLayer = "CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i + "].PD.Charge.Layers.Actual",
                
                IsDischarge = "CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i + "].PD.Tray.Functions.Discharge",
                IsWatch = "CPU2.PLC.Blocks.01 Tray Handling.03 HZ.DB HZ PD.Place[" + i + "].PD.Tray.Functions.Watch",
                Station = 13,
                Place = i,
                Coord= i ,
                Type = "Tray",
