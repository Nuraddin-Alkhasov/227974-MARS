    ...
    public partial class MO_CZ
    {
        ...
        private void AddObjects_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i <= 7; i++)
            {
                ...
                uint ChargeNr = (uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.04 CZ.DB CZ PD.Place[" + i.ToString() + "].PD.Charge.Data.Charge");
                ...
                if (ChargeNr == 1)
                {
                    Dispatcher.InvokeAsync(delegate
                    {
                        ...
                        {
                            Data = new Track((uint)ApplicationService.GetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.04 CZ.DB CZ PD.Place[" + i.ToString() + "].PD.Header.BoxId"))
                        };
                        switch (Coord)
                        {
