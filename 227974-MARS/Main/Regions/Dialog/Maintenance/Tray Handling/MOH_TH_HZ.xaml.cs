        ...
        private void Reset_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            if (MessageBoxView.Show("@Maintenance.Text15", "@Maintenance.Text16", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                ...
                if (btn1.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.05 Airflow Heating.04 HZ.DB HZ Air HMI.Actual.Circulating air.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.05 Airflow Heating.04 HZ.DB HZ Air HMI.Actual.Circulating air.Operating hours.Reset", false);
                ...
                if (btn2.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.05 Airflow Heating.04 HZ.DB HZ Air HMI.Actual.Burner.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.05 Airflow Heating.04 HZ.DB HZ Air HMI.Actual.Burner.Operating hours.Reset", false);
