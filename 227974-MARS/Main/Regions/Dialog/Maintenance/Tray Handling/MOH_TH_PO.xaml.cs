        ...
        private void Reset_Click(object sender, System.Windows.RoutedEventArgs e)
        {
            if (MessageBoxView.Show("@Maintenance.Text15", "@Maintenance.Text16", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                ...
                if (btn1.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.05 PO.01 PO Traction.DB PO Traction HMI.Actual.Traction.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.05 PO.01 PO Traction.DB PO Traction HMI.Actual.Traction.Operating hours.Reset", false);
                ...
                if (btn2.IsSelected)
                {
                    ...
                    Task taskA = Task.Run(() =>
                    {
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.05 PO.02 PO Turn.DB PO Rotary HMI.Actual.Rotary.Operating hours.Reset", true);
                    });
                    taskA.ContinueWith(async x =>
                    {
                        await Task.Delay(1000);
                        ApplicationService.SetVariableValue("CPU2.PLC.Blocks.01 Tray Handling.05 PO.02 PO Turn.DB PO Rotary HMI.Actual.Rotary.Operating hours.Reset", false);
